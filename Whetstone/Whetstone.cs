...
namespace Whetstone
{
	...
	{
		public const float CraftTime = 0.25f;
		...
		public override string OnUsed(Player player, ItemStack itemStack)
		{
			...
			{
				repairable.Durability = Math.Min(100, repairable.Durability + 25);
				player.InfoBox(Localizer.DoStr($"{selectedItem.DisplayName} repaired by 25%!"));
			});
			return base.OnUsed(player, itemStack);
		}
	...
	public class WoodWhetstoneRecipe : RecipeFamily
	{
		...
		public WoodWhetstoneRecipe()
		{
			...
			ExperienceOnCraft = 1;
			LaborInCalories = CreateLaborInCaloriesValue(50);
	...
	public class IronWhetstoneRecipe : RecipeFamily
	{
		public IronWhetstoneRecipe()
		{
			...
			Recipes = new List<Recipe> { recipe };
			ExperienceOnCraft = 1;
			LaborInCalories = CreateLaborInCaloriesValue(75, typeof(SmeltingSkill));
			CraftMinutes = CreateCraftTimeValue(
